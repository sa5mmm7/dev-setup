...
Runs the full Windows/WSL2 onboarding sequence, or a single step.
...
install_project_tool() {
  ...
  if append_zshrc 'export PATH="${HOME}/.local/bin:${PATH}"'; then
    ...
    echo "[WARN] ${profile} was updated — run: source ${profile}  (or open a new terminal)"
  fi
...
run_all() {
  ...
  if [[ "${ZSHRC_UPDATED:-false}" == "true" ]]; then
    ...
    profile="$(shell_profile)"
    echo ""
    echo "⚠️  ${profile} was updated during this run — run: source ${profile}  (or open a new terminal)"
  fi
}
